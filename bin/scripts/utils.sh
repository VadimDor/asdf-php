...
# this script derived from https://github.com/GhostWriters/DockSTARTer/blob/master/main.sh
# good intro for right shell programming is here
# https://github.com/GhostWriters/DockSTARTer/blob/master/.github/CONTRIBUTING.md#shell-scripts
...
NOTE: shortcut is only available after the first run of
    bash main scripts .sh
...
cleanup() {
    ...
    sudo sh -c "cat ${MKTEMP_LOG:-/dev/null} >> ${SCRIPTPATH}/script.log" || true
    sudo rm -f "${MKTEMP_LOG}" || true
    sudo sh -c "echo \"$(tail -1000 "${SCRIPTPATH}/script.log")\" > ${SCRIPTPATH}/script.log" || true
