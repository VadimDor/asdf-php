...
export sriptID="srgwrgytujy8j5bb6n79k"   #any random id
if [[ -z "$(cat $HOME/.bashrc | grep $sriptID )" ]]; then
...
asdf plugin add php
sed -i 's+https\:\/\/github.com\/php\/php-src\/archive+https\:\/\/github.com\/php\/php-src\/refs\/tags+g' /etc/apt/sources.list
